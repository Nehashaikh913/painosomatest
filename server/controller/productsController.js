...
const updateproduct = asyncHandler(async (req, res) => {
    const { product_name, product_price, product_slug, product_link, strength,parentcategory,subcategory, othercompany, otherprice, aboutheader, abouteditor, newsheader, newseditor, advanceheader, advanceeditor,segregation,referenceeditor,heading,faqeditor,title,description, status,date} = req.body;
    const id = req.params.id;
    const feature_image = req.file ? req.file.filename : req.body.image
    con.query('UPDATE `products` SET  image=?, product_name=?, product_price=?, product_slug=?, product_link=?, strength=?,parentcategory=?,subcategory=? , othercompany=?, otherprice=?, aboutheader=?, abouteditor=?, newsheader=?, newseditor=?, advanceheader=?, advanceeditor=?,segregation=?,referenceeditor=?,heading=?,faqeditor=?,title=?,description=?, status=? ,date=? WHERE id IN (?)',
        [feature_image, product_name, product_price, product_slug, product_link, strength,parentcategory,subcategory, othercompany, otherprice, aboutheader, abouteditor, newsheader, newseditor, advanceheader, advanceeditor,segregation,referenceeditor,heading,faqeditor,title,description, status,date, id], (err, result) => {
            if (err) console.log(err)
            //  res.send(result)
        })
