...
import { addBlogApi, deleteBlogApi, getblogApi, updateBlogApi } from "../api/blog";
import { getGalleryApi } from "../api/gallery";
import { getAuthorsApi } from "../api/author";
import { getParentCategoryApi, getSubCategoryApi } from "../api/category";
...
function Blogs() {
    ...
    async function getAuthor() {
        const Authordata = await getAuthorsApi();
    ...
    const addupdateBlogFunction = async (data) => {
        ...
        if (blog.id) {
            await updateBlogApi(data.id,formData)
        } else {
